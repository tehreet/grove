...
// === Mulch Records (for directive rendering) ===

export type MulchRecordClassification = "foundational" | "tactical" | "observational";

export interface MulchRecordOutcome {
...
/**
 * A structured mulch expertise record for directive rendering.
 * Mirrors the ExpertiseRecord type from @os-eco/mulch-cli.
 */
export type MulchRecord =
	| {
			type: "convention";
			content: string;
...
export interface OverlayConfig {
	...
	/** Structured mulch records for directive rendering. When provided, formatMulchExpertise() splits into directives + context sections. */
	mulchRecords?: MulchRecord[];
	/** When true, skip directive rendering and use the flat format (for A/B testing via --no-directives). */
