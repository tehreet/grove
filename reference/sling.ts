...
import type { AgentSession, MulchRecord, OverlayConfig } from "../types.ts";
...
export interface SlingOptions {
	...
	baseBranch?: string;
	/** When true, skip directive rendering and use the flat mulch expertise format (for A/B testing). */
...
export async function slingCommand(taskId: string, opts: SlingOptions): Promise<void> {
	...
	try {
		...
		try {
			...
			// 8a. Compute mulch domains once — used for both prime and structured record fetch
			const mulchDomains = config.mulch.enabled
			...
			// 8c. Fetch structured mulch records for directive rendering.
			// Uses the programmatic API directly to get typed records per domain.
			// Non-fatal: directive rendering falls back to flat format on any error.
			let mulchRecords: MulchRecord[] | undefined;
			if (config.mulch.enabled && mulchDomains.length > 0 && !(opts.noDirectives ?? false)) {
				try {
					const MULCH_PKG = "@os-eco/mulch-cli";
					const mulchApi = (await import(MULCH_PKG)) as unknown as {
						queryDomain(domain: string, opts?: { cwd?: string }): Promise<MulchRecord[]>;
					};
					const allRecords: MulchRecord[] = [];
					for (const domain of mulchDomains) {
						const recs = await mulchApi.queryDomain(domain, { cwd: config.project.root });
						allRecords.push(...recs);
					}
					mulchRecords = allRecords.length > 0 ? allRecords : undefined;
				} catch {
					// Non-fatal: directive rendering falls back to flat format
					mulchRecords = undefined;
				}
			}
			...
			const overlayConfig: OverlayConfig = {
				...
				mulchExpertise,
				mulchRecords,
				noDirectives: opts.noDirectives ?? false,
